...
Ожидаемые файлы для версии 1.9.3:

- `release/Codex-Account-Manager-Setup-1.9.3.exe`
- `release/Codex-Account-Manager-1.9.3.exe`
- `release/latest.yml`
- `release/Codex-Account-Manager-Setup-1.9.3.exe.blockmap`
- `release/SHA256SUMS-1.9.3.txt`

## Контрольные суммы

```powershell
Get-ChildItem release -File |
  Where-Object { $_.Name -match '1\.9\.3|latest\.yml|blockmap' } |
  Get-FileHash -Algorithm SHA256 |
  ForEach-Object { "$($_.Hash.ToLower())  $([IO.Path]::GetFileName($_.Path))" } |
  Set-Content release/SHA256SUMS-1.9.3.txt -Encoding ascii
```
